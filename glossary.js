...
const SCALE_NAMES_PATTERNS = Object.freeze([
    Object.freeze([SCALE_NAME_MAJOR,            SCALE_PATTERN_MAJOR]),
    Object.freeze([SCALE_NAME_HARMONIC_MINOR,   SCALE_PATTERN_HARMONIC_MINOR]),
    Object.freeze([SCALE_NAME_HARMONIC_MAJOR,   SCALE_PATTERN_HARMONIC_MAJOR]),
    Object.freeze([SCALE_NAME_MELODIC_MINOR,    SCALE_PATTERN_MELODIC_MINOR]),
    Object.freeze([SCALE_NAME_DOUBLE_HARMONIC,  SCALE_PATTERN_DOUBLE_HARMONIC]),
    Object.freeze([SCALE_NAME_NEAPOLITAN_MINOR, SCALE_PATTERN_NEAPOLITAN_MINOR]),
    Object.freeze([SCALE_NAME_NEAPOLITAN_MAJOR, SCALE_PATTERN_NEAPOLITAN_MAJOR])
]);
...
const scalePatternToOffsetPattern = (scalePattern) => {
    return scalePattern.split('').map(Number).reduce((acc, elem, i) => {
        if (elem === 1) {
            acc.push(i);
        }
        return acc;
...
const offsetsPatternsScalesNames = SCALE_NAMES_PATTERNS.reduce((acc, elem) => {
    acc.set(scalePatternToOffsetPattern(elem[1]).join(','), elem[0]);
    return acc;
}, new Map());

const offsetsPatternsModesNames = SCALES_NAMES_MODES_NAMES.reduce((acc, elem) => {
    elem[2].forEach((elemModeName, j) => {
        acc.set(elem[0].getModeOffsets(j).join(','), elemModeName);
    });
    return acc;
}, new Map());
